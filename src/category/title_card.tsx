...
const TitleCard:React.FC<TitleCardProps>=({category})=>{
    ...
    <div className='articles-wrapper'>
        <h1 className="articles-heading">{category}の記事一覧</h1>
                    ...
                    <Link key={post.id} to={`/post/${post.id}`} className="article-card">
                        ...
                        <div className="article-card-body">
                            ...
                            <div className="article-card-actions">
                                <button onClick={(e) => {e.stopPropagation(); handleDelete(post.id); }} className="article-card-delete">削除</button>
                            </div>

                        </div>

                        {/* 右側：サムネイル画像（画像がある場合のみ表示、ない場合は空白） */}
                        {post.image_url && (
